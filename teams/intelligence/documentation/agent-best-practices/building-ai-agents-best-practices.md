...
**How do you know when to stop? Two tests.**

**First, the manual test:** could you write step-by-step instructions for how to accomplish this task? If the instructions remain vague ("research the account and write something relevant"), you haven't decomposed enough.

**Second, the test case test:** can you write twenty specific input-output pairs that define success? If you can only write three or four, the scope is still too broad.
...
1. **Can I describe this as a black box with clear inputs and outputs?** If the boundary is fuzzy, the implementation will be fuzzy.

2. **Can I explain the step-by-step process to accomplish this task, or articulate the principles that should guide how it's done?** If I handed these instructions to an intern, could they execute?

3. **Can I write twenty test cases that define success?** If not, the scope is too broad or I don't understand it well enough.

4. **What data sources are required?** Can I justify why each one is necessary and what context it provides?

5. **What queries should this agent reject?** If everything is in scope, nothing is.

6. **How will users know if it's working?** What does success look like from their perspective?

7. **What's the worst-case failure mode?** What happens when it goes wrong, and how will we catch it?

If you can't answer these questions clearly, you're not ready to build. And that's okay. The process of answering them is the actual product work.

## Conclusion

Building AI agents requires a fundamental shift in how we think about product management. We're no longer specifying deterministic features. We're shaping probability spaces, designing boundaries, and managing expectations around where success is required versus where failure is acceptable.

The key disciplines:
- **Decompose** broad capabilities into bounded, testable problems
- **Define** explicit inputs, outputs, processes, and boundaries
- **Test** your design with real test cases before implementation
- **Communicate** limitations clearly to users and stakeholders
- **Shape** probability space strategically: rigidity where it matters, flexibility where it doesn't

This is harder than traditional product management in some ways. It requires deeper thinking about use cases, more explicit requirement writing, and constant awareness of probabilistic failure modes. But it's also more powerful. When done well, AI agents can handle natural language inputs, adapt to novel situations, and provide value at scale in ways traditional software never could.

The question isn't whether to build agents. The question is how to build them well. This document provides the foundation. The rest is execution.
